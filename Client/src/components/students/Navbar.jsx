...
const Navbar = () => {
    ...
    return (
        <div className={`flex items-center justify-between px-4 sm:px-10 md:px-14 lg:px-36 border-b border-gray-500 py-4 ${isCourseListPage ? 'bg-white' : 'bg-cyan-100/70'}`}>
            <img onClick={() => navigate('/')} src={assets.logo} alt="Logo" className='w-32 lg:w-32 cursor-pointer' />
            <div className='hidden md:flex items-center gap-5 text-gray-500'>
                ...
                {user ? <UserButton /> :
                    ...
                    <button onClick={() => openSignIn()} className='bg-blue-600 text-white cursor-pointer px-5 py-2 rounded-full '>Create Account</button>}

            </div>

            {/* phone */}
            <div className='md:hidden flex itms-center gap-2 sm:gap-5 text-gray-500'>
